...
const Comments: FC<Props> = ({ belongsTo, fetchAll }): JSX.Element => {
  ...
  const updateEditedComment = (newComment: CommentResponse) => {
    ...
    // To update the comment, we can only change the content if edited comment is chief
    ...
    // Else from comment from replies and update comment from replies.
    else {
      const chiefCommentIndex = updatedComments.findIndex(
        ...
        return (
          <div key={comment.id}>
            <CommentCard
              ...
              onLikeClicked={() => handleOnLikeClick(comment)}
            />

            {replies?.length ? (
              <div className="w-[93%] ml-auto space-y-3 pt-4">
                <h1 className="text-secondary-dark mb-3 font-semibold">
                  Replies
                </h1>
                {replies.map((reply) => {
                  return (
                    <CommentCard
                      ...
                      onLikeClicked={() => handleOnLikeClick(reply)}
                    />
                  );
                })}
              </div>
