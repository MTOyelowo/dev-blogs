...
const createNewComment: NextApiHandler = async (req, res) => {
    ...
    if (error) return res.status(442).json({ error });
...
const removeComment: NextApiHandler = async (req, res) => {
    ...
    // if chief comment is removed, other related comments i.e. replies are also removed.

    //if a reply is removed, only that reply is removed from the chief comment replies
    ...
    if (comment.chiefComment) await Comment.deleteMany({ repliedTo: commentId });
        ...
        // if this is not the chiefComment, remove from the chiefComments replies section.
